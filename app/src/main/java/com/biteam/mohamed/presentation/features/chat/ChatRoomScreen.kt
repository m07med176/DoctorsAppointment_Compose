...
import com.biteam.mohamed.presentation.features.appointment.sections.ButtonImageHolderDoctorProfile
import com.biteam.mohamed.presentation.theme.SystemColor


data class ChatRoomScreen(val model: MessageModel) : Screen {
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        ChatRoomContent(model, navigator)
    }
}
...
@Composable
fun ChatRoomContent(model:MessageModel, navigator: Navigator? = null) {
    ...
    Scaffold(topBar = {
        ...
        TopAppBar(
            ...
            Box(modifier = Modifier.weight(0.8f), contentAlignment = Alignment.Center) {
                TextHead1(text = model.name )
            }

            ButtonImageHolderDoctorProfile(
                R.drawable.voice_call_ic,
                simpleClick = {
                    navigator?.push(AudioCallScreen(model))
